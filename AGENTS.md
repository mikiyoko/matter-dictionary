...
### 法6: 描画・操作に関わる変更は push 前にローカル実描画で検証する
- JS の描画・イベント・状態管理（カード生成、モーダル、フィルター、localStorage、onclick 等）に
  触れる変更は、**静的チェック（構文チェック・grep）だけで push しない**。必ずローカルの
  HTTP サーバーで実際にブラウザ描画・実操作まで確認してから push すること。
  ```bash
  # data/*.json を fetch するため file:// では動かない。必ず HTTP で配信する。
  cd matter-dictionary && python3 -m http.server 8765 --bind 127.0.0.1
  # → http://127.0.0.1:8765/index.html を開き、変更箇所を実操作で確認
  ```
- 最低限、次を目視で確認する: カードが実際に描画されるか／変更したボタン・操作が反応するか／
  コンソールにエラーが出ていないか／JA・EN 両方。
- 背景（この法ができた理由）: 「初期化順で `FAVS` 未定義 → 描画関数が例外終了しカード全消失」
  「`onclick` が `initApp` 内ローカル関数を参照できず無反応」という2件の runtime バグは、
  いずれも構文チェックは通るが実描画では壊れていた。静的検証だけでは検出できない。
- 自動化する場合は Chrome を headless で起動し、同一オリジンのハーネスから実クリックまで
  検証してよい（`--headless=new --dump-dom` で状態を回収）。ハーネスは一時ファイルとし commit しない。
